...
const MockExam = (() => {
  ...
  const CONTEXT_TEMPLATES = {
    n5: [
      {s:'＿＿について話しました。',filter:c=>c==='名',category:'通用'},
      {s:'＿＿は面白いです。',filter:c=>c==='名',category:'通用'},
      {s:'＿＿は役に立ちます。',filter:c=>c==='名',category:'通用'},
    ...
    n3: [
      ...
      {s:'＿＿は非常に重要です。',filter:c=>c==='名',category:'通用'},
      {s:'＿＿はとても大切です。',filter:c=>c==='名',category:'通用'},
      {s:'天気が＿＿ので、出かけましょう。',filter:c=>c==='い形'||c==='な形',category:'天気'},
    ],
    n2: [
      {s:'＿＿について深く考えた。',filter:c=>c==='名',category:'通用'},
      {s:'＿＿に関する報告書を提出した。',filter:c=>c==='名',category:'主題'},
      {s:'＿＿は彼女の強みです。',filter:c=>c==='名',category:'通用'},
    ],
    n1: [
      {s:'＿＿は社会に大きな影響を与える。',filter:c=>c==='名',category:'通用'},
      {s:'＿＿は今後の課題です。',filter:c=>c==='名',category:'通用'},
    ],
  };
  ...
  function makeUsageSentence(word, isCorrect, replaceWith) {
    const templates = {
      // 名詞「昨日{w}を見ました」太 specific（需可見的詞），改成「{w}を勉強しています」更通用
      '名': ['{w}はとても大切です。', '{w}を勉強しています。', '{w}が好きです。', '{w}について話しました。'],
      '動': ['{w}ことが大好きです。', '毎日{w}ています。', 'もっと{w}たいです。'],
      // い形「{w}天気ですね」太 specific（需形容天氣的詞），改成「{w}本を読みました」更通用
      'い形': ['この部屋は{w}です。', 'とても{w}料理でした。', '{w}本を読みました。'],
  ...
  function start() {
    ...
    if (typeof cvtStaticUI === 'function') cvtStaticUI(box);   // 簡中:轉設定框寫死的繁體字(無日文,安全)
